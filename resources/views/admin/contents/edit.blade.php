...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">@lang('contents/title.edit')</li>
    </ol>
</section>
                ...
                <div class="panel-body">
                    <form class="form-horizontal" role="form" method="post" action="" enctype="multipart/form-data">
                        ...
                        @foreach($langs as $lang)
                            <?php $degisken = 'title_'.$lang->kisaltma; ?>
                            <div class="form-group {{ $errors->first($degisken, 'has-error') }}">
                                ...
                                <div class="col-sm-5">
                                    <input type="text" id="{{ $degisken }}" name="{{ $degisken }}" class="form-control" placeholder="" value="{{{ Input::old($degisken, $content->$degisken)  }}}">
                                </div>
                                <div class="col-sm-4">
                                    {!! $errors->first($degisken, '<span class="help-block">:message</span> ') !!}
                                </div>
                            </div>
                            <?php $degisken = 'content_'.$lang->kisaltma; ?>
                            <div class="form-group {{ $errors->first($degisken, 'has-error') }}">
                                <label for="{{ $degisken }}" class="col-sm-2 control-label">
                                    @lang('contents/form.content') [{{ strtoupper($lang->kisaltma) }}]
                                </label>
                                <div class="col-sm-5">
                                    <textarea class="form-control" id="ckeditor_full" name="{{ $degisken }}">{{{ Input::old($degisken, $content->$degisken) }}}</textarea>
                                </div>
                                <div class="col-sm-4">
                                    {!! $errors->first($degisken, '<span class="help-block">:message</span> ') !!}
                                </div>
                        ...
                        <div class="form-group {{ $errors->first('status', 'has-error') }}">
                            ...
                            <div class="col-sm-2">
                                <select class="form-control" id="status" name="status">
                                    <option value="1" @if($content->status === 1) selected="selected" @endif>Aktif</option>
                                    <option value="0" @if($content->status === 0) selected="selected" @endif>Pasif</option>
                                </select>
                            </div>
                            <div class="col-sm-2">
